...
public class LongTwoQualified_DecoratorAwareBeanFactoryPostProcessorTest {
	...
	public void testChaining_MustBeTwoChains() {
		...
		Assert.isTrue(((ResolverCDIAutowiringRules) resolver.getPlugins()
				.iterator().next()).getDecoratorChains().size() == 2);
	}
	...
	public void testChaining_MyDelegateMustBeDelegate() {
		...
		DecoratorAwareAutowireCandidateResolver resolver = (DecoratorAwareAutowireCandidateResolver) ((DefaultListableBeanFactory) beanFactory)
				.getAutowireCandidateResolver();
		Assert.isTrue(((ResolverCDIAutowiringRules) resolver.getPlugins()
				.iterator().next()).getDecoratorChains().get(1)
				.getDelegateBeanDefinitionHolder().getBeanName()
				.equals("longTwoQualified_MyDelegate"));
	}
	...
	public void testChaining_AnotherDelegateMustBeDelegate() {
		...
		Assert.isTrue(((ResolverCDIAutowiringRules) resolver.getPlugins()
				...
				.getDelegateBeanDefinitionHolder().getBeanName()
				.equals("longTwoQualified_AnotherDelegate"));
	}
	...
	public void testChaining_MustBeThreeDecoratorsInBothChains() {
		...
		Assert.isTrue(((ResolverCDIAutowiringRules) resolver.getPlugins()
				...
				.size() == 2);
		Assert.isTrue(((ResolverCDIAutowiringRules) resolver.getPlugins()
				.iterator().next()).getDecoratorChains().get(1).getDecorators()
				.size() == 2);
	}
